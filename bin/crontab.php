...
if($_SERVER["argv"][1]){
	if (function_exists( $_SERVER["argv"][1] )){
		echo "Manually run: ".$_SERVER["argv"][1]."\n";
		eval ( "\$ret = ".$_SERVER["argv"][1]."();" );
		echo "Status: ".$ret[0].", Info: ".$ret[1]."\n\n";
		exit;
	}
}
...
echo " check: ".$row["content_crontab__name"]." - function: ".$row["content_crontab__exec"]."\n";
